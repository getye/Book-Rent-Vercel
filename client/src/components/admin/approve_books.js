...
export const ApproveBooks = () => {
    ...
    const handleAccept = async (book_id) => {
        try {
            const response = await fetch(`${window.location.origin}/admin/approve/book/accept/${book_id}`, {
                method: 'PUT',
            });
    ...
    const handleReject = async (book_id) => {
        try {
            const response = await fetch(`${window.location.origin}/admin/approve/book/reject/${book_id}`, {
                method: 'PUT',
            });
    ...
    return (
        <Box sx={{ marginLeft: {xs:'1%', sm:'8%', md:'15%', lg:'20%'}, paddingRight:'8%', paddingTop: 10, justifyContent: 'center' }}>
            <Box sx={{ paddingTop: 2 }}>
                <Paper
                    sx={{ 
                        display: 'flex', 
