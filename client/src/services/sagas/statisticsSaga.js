...
function* fetchBookCounts() {
  try {
    ...
    const response = yield call(fetch, `${window.location.origin}/owner/dashboard/book-types`, {
      method: 'GET',
      headers: {
...
function* fetchEarningsSaga() {
  try {
    ...
    const response = yield call(fetch, `${window.location.origin}/owner/dashboard/monthly-earnings`, {
      method: 'GET',
...
function* fetchRents() {
  try {
    ...
    const response = yield call(fetch, `${window.location.origin}/renter/dashboard/book-types`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`, // Include token in the header
      }
...
function* fetchRentFee() {
  try {
    const token = localStorage.getItem('token'); // Retrieve token from localStorage
    const response = yield call(fetch, `${window.location.origin}/renter/dashboard/monthly-fees`, {
      method: 'GET',
      headers: { 'Authorization': `Bearer ${token}` }
    });
    const data = yield response.json();
