...
function* submitRentalSaga(action) {
    ...
    try {
      ...
      const response = yield call(axios.post, `${window.location.origin}/renter/book/rent`, action.payload, {
        headers: {
  ...
  function* fetchRentsSaga() {
    try {
        const response = yield call(axios.get, `${window.location.origin}/admin/approve/rents`);
...
function* acceptRentSaga(action) {
    ...
    try {
        const response = yield call(axios.put, `${window.location.origin}/admin/approve/rent/accept`, { renter_id, book_id });
...
function* rejectRentSaga(action) {
    const { renter_id, book_id } = action.payload;
    try {
        const response = yield call(axios.put, `${window.location.origin}/admin/approve/rent/reject`, { renter_id, book_id });
